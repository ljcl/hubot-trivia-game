  ...
  checkScore: (resp, name) ->
    if name == "all"
      ...
      for user in userList
        user.triviaScore = user.triviaScore or 0
		#prevent division by 0 for someone that hasn't played
		user.triviaAnswers = user.triviaAnswers or 1
		user.triviaCorrect = user.triviaCorrect or 0
		correctPercentage = (user.triviaCorrect / user.triviaAnswers * 100).toFixed(2)
